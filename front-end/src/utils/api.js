...
const { REACT_APP_API_BASE_URL = "http://localhost:5000" } = process.env;
...
export async function createReservation(newRes, signal) {
  const url = new URL(`${API_BASE_URL}/reservations`);
...
export async function createTable(newTab, signal) {
  ...
  return await fetchJson(
    url,
    { body: JSON.stringify({ data: newTab }), headers, method: "POST", signal },
    []
  );
}
...
export async function editRes(editRes, signal) {
  const url = new URL(`${API_BASE_URL}/reservations/${editRes.reservation_id}`);

  return await fetchJson(
    url,
    {
      body: JSON.stringify({ data: editRes }),
      headers,
      method: "PUT",
      signal,
    },
